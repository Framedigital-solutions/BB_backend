...
export default function ProductPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const c = await apiFetch('/api/cart');
        setCart(c || { items: [] });
      } catch (err) {
        console.warn('load cart', err.message || err);
      }
    })();
  }, []);
  ...
  const removeItem = async (productId) => {
    try {
      const res = await apiFetch('/api/cart/remove', { method: 'POST', body: JSON.stringify({ productId }) });
